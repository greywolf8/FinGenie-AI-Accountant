...
// Create a new chat
export async function createNewChat(did: string, title = "FinGenie AI Assistant"): Promise<string | null> {
  try {
...
export async function saveChatMessage(
  ...
  try {
    if (chatId) {
      ...
      // If this is the first message from the bot (intro message), use it as the chat title
      if (message.role === "bot" && messages.length === 1) {
        // Use the bot's intro message as the chat title (truncate if too long)
      ...
      // If this is a user message and it's the second message (after intro), update the chat title
      else if (message.role === "user" && messages.length === 2) {
