...
import { Plus, ArrowLeft, Download, Upload, Database, Calculator } from "lucide-react"
...
export default function ChatManager({ onClose }: ChatManagerProps) {
  ...
  // Add a state variable to force refresh of chat history
  ...
  // Update the handleNewChat function to refresh the chat history after creating a new chat
  const handleNewChat = async () => {
    ...
    const chatId = await createNewChat(user.did, "FinGenie AI Assistant")
    if (chatId) {
      setActiveChat(chatId)
      // Force the history to show when creating a new chat
      setShowHistory(true)
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="bg-gradient-to-r from-orange-500 to-pink-500 p-4 text-white flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {showTaxCalculator ? "Back to Chat" : "Tax Calculator"}
          </Button>
          <Button
            variant="ghost"
            size="sm"
      ...
      <div className="flex flex-1 overflow-hidden">
        {showHistory && !showTaxCalculator && (
          <div className="w-64 border-r border-gray-200 dark:border-gray-800 overflow-y-auto">
            <ChatHistory onSelectChat={handleSelectChat} forceRefresh={forceRefresh} />
          </div>
        )}
        <div className="flex-1">
          {showTaxCalculator ? (
            <TaxCalculator onClose={() => setShowTaxCalculator(false)} onSendToChat={handleSendToChat} />
          ) : (
